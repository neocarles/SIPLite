...
Begin VB.Form Cetak_Pemasukan 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   BorderStyle     =   0  'None
   Caption         =   "Cetak Data Pemasukan"
   ClientHeight    =   8730
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   13620
   Icon            =   "Cetak_Pemasukan.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   8730
   ScaleWidth      =   13620
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "Preview atau Cetak Data Pemasukan Yang Tersedia."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.OptionButton OptAll 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      Caption         =   "Cetak Semua Data Pemasukan"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
   ...
   Begin VB.Image Image1 
      Height          =   1350
      Left            =   300
      Picture         =   "Cetak_Pemasukan.frx":03A6
      Stretch         =   -1  'True
      Top             =   960
      Width           =   1350
   End
   ...
   End
End
Attribute VB_Name = "Cetak_Pemasukan"
...
Private Sub ButtonPreview_Click()
    ...
    Unload DataPemasukan

    If OptTanggal.Value = True Then
        MyRS.Open "SELECT kode,tanggal,keterangan,kode_kas,jumlah" & _
                " FROM pemasukan WHERE tanggal BETWEEN '" & Format(Tanggal1.Value, "yyyy-mm-dd") & "' and '" & Format(Tanggal2.Value, "yyyy-mm-dd") & "'", Conn, adOpenForwardOnly, adLockReadOnly
        strSQL = "SELECT kode,tanggal,keterangan,kode_kas,jumlah" & _
               " FROM pemasukan WHERE tanggal BETWEEN '" & Format(Tanggal1.Value, "yyyy-mm-dd") & "' and '" & Format(Tanggal2.Value, "yyyy-mm-dd") & "'"
        strData = MyRS.RecordCount
    ElseIf OptAll.Value = True Then
        MyRS.Open "SELECT kode,tanggal,keterangan,kode_kas,jumlah from pemasukan ORDER BY tanggal ASC", Conn, adOpenForwardOnly, adLockReadOnly
        strSQL = "SELECT kode,tanggal,keterangan,kode_kas,jumlah from pemasukan ORDER BY tanggal ASC"
        strData = MyRS.RecordCount
    Else
        MyRS.Open "SELECT kode,tanggal,keterangan,kode_kas,jumlah from pemasukan ORDER BY tanggal ASC", Conn, adOpenForwardOnly, adLockReadOnly
        strSQL = "SELECT kode,tanggal,keterangan,kode_kas,jumlah from pemasukan ORDER BY tanggal ASC"
